...
#         log_to_cloudwatch("Fnal testing")
...
from utils.db import index_data_in_db, search_data_in_db
from middleware.api_key import get_current_api_key
...
@app.post("/index/", tags=["indexing"])
async def index_data(data: QA, api_key: str = Depends(get_current_api_key)) -> dict:
    return await index_data_in_db(data)

@app.post("/search/", tags=["search"])
async def search(data: Question, api_key: str = Depends(get_current_api_key)) -> dict:
    return await search_data_in_db(data)
